...
class PlacesRepository constructor(
    private val apiService: APIService, db: WeatherDB,
    private val appPrefs: IPrefsInteractor, private val assetManager: AssetManager
) {
    ...
    fun updateCurrentPlacesForecast(): Completable {
        ...
        return weatherDAO
            ...
            .andThen(
                placeDAO.getCurrentCityId().zipWith(
                    appPrefs.getLocale(),
                    zipper = { id, locale -> Pair(id, locale.language) }
                ).flatMap { idAndLang ->
                    val (id, lang) = idAndLang
                    apiService.getForecastAsync(
                    ...
                    .flatMapCompletable { placeAndWeathers ->
                        ...
                        placeDAO.updateSunsetSunrise(
                            updatePlaceEntry.id,
                            updatePlaceEntry.timezone,
                            updatePlaceEntry.sunrise,
                            updatePlaceEntry.sunset
                        )
        ...
        Flowable.create({ emitter ->
            ...
            val placeEntries = PlaceEntryMapper()
                .map(places,
                    { relation ->
                        val percent =
                            (percentAfterGSONMapping + mappingToEntriesPart * relation).toInt()
