...
import com.lab.esh1n.weather.presentation.model.NoDataBackgroundModel
import com.lab.esh1n.weather.presentation.model.PlaceModel
import com.lab.esh1n.weather.presentation.model.SimpleBackgroundModel
import java.util.*
...
class PlaceWeatherListMapper(private val uiLocalizer: IUILocalisator) :
    ...
    override fun map(source: PlaceWithCurrentWeatherEntry): PlaceModel {
        ...
        val isDay =
            if (source.iconId == null) dateBuilder.getHour24Format() in 9..18 else WeatherModelMapper.isDay(
                source.iconId
            )
        val weatherBackgroundModel =
            if (source.iconId == null) NoDataBackgroundModel(isDay = isDay) else SimpleBackgroundModel(
                source.iconId, isDay = isDay, hourOfDay = DateBuilder(
                    source.date
                        ?: Date(), source.timezone
                ).getHour24Format(),
