...
abstract class PlaceDAO {
    ...
    @Query("SELECT id,placeName,iconId,temperatureMax,measured_at,timezone,dateTxt,rain,cloudiness,snow  FROM place  INNER JOIN weather w ON place.id = w.placeId AND w.measured_at = (SELECT measured_at FROM weather innerW WHERE  innerW.placeId = w.placeId ORDER BY abs(:now - measured_at) ASC LIMIT 1)")
