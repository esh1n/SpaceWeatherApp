...
import com.esh1n.core_android.retrofit.auth.TokenProvider
...
class OkHttpBuilder private constructor(endPoint: String) {
    ...
    fun withAuthorizationHeaderInterceptor(tokenProvider: TokenProvider): OkHttpBuilder {
        mClientBuilder.addInterceptor { chain ->
            val request = chain.request()
            val builder = request.newBuilder()
            val token = tokenProvider.token
            chain.proceed(addAuthorizationHeader(builder, token).build())
